...
int main(int argc, const char** argv)
{
	...
	TextDiagnosticPrinter *DiagClient = new TextDiagnosticPrinter(llvm::errs(), clang::DiagnosticOptions());
	...
	TargetInfo *pti = TargetInfo::CreateTargetInfo(AnnotationCompiler.getDiagnostics(), to);
	...
	const FileEntry *pFile = AnnotationCompiler.getFileManager().getFile(argv[1]);
	...
	AnnotationCompiler.getDiagnosticClient().BeginSourceFile(AnnotationCompiler.getLangOpts(),
	                                            &AnnotationCompiler.getPreprocessor());
	Parser parser(AnnotationCompiler.getPreprocessor(), AnnotationCompiler.getSema(), false /*skipFunctionBodies*/);
