...
/**
  Sriram Sankaranarayanan:
   ...
   The other I leave as a challenge to the
...
/*  for use in the far distant future, convert a packed field to a large
    ...
    to accommodate multiplication (once!).
*/
...
/*  subtract two BIGINTS (c = a - b <=> c = a + -b).
    As in addition, c can point to a or b and it still works.
*/
...
/*  multiply two BIGINTs to get a third.
    ...
    to do this.
    c can *not* be one of a or b, it must be a separate storage location.
*/
...
BIGINT *top, *bottom, *quotient, *remainder;
{
    ...
    // check for error inputs, does not check for zero, so it's 
    // actually incorrect.
    if (!m) // x / 1 == x
    {
...
BIGINT *outhex;
{
    ...
    int_null( &ten); // decimal multiplier
    ten.hw[INTMAX] = 0xA;
    int_null( &digit);
    int_null( outhex);
...
/*  convert binary BIGINT to ASCII string. Assumes destination has 
    ...
    or about 5/4*HALFSIZE*MAXLONG chars. Works backwards and blank-fills
    destination string.
*/
...
/*--
  SS: The harness function. Initializes two
  BIGINTs in some way and does some functions
  on them based on the input specification.
  After that, frees them.
  --*/
...
int main(int cmd1, int zer, int cmd2) {
   ...
   switch (cmd1) {
      ...
      case COPY_INIT:
         ...
         int_copy(a,b);
      case ALIAS_INIT:
         a = b = (BIGINT*) malloc(sizeof(BIGINT));
         if (zer > 0) int_null(a);
         break;
      default:
         return -1;
   ...
   free(a);
   free(b); // SS: possible bug here, e.g., if we chose to alias a to b
   free(c);
   free(d);
