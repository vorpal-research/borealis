...
void OutOfBoundsChecker::visitGEPOperator(llvm::Instruction& loc, llvm::GEPOperator& GI) {
    ...
    if (not module_->checkVisited(&loc) || not module_->checkVisited(&GI)) {
        ...
        for (auto j = GI.idx_begin(); j != GI.idx_end(); ++j) {
            ...
            AbstractDomain::Ptr indx = intConstant ?
                               module_->variableFactory()->af()->getMachineInt(*intConstant->getValue().getRawData()) :
                               module_->getDomainFor(llvm::cast<llvm::Value>(j), loc.getParent());

            offsets.emplace_back(module_->variableFactory()->af()->machineIntInterval(indx));
        }
    ...
    try {
        ...
        for (auto i = 0U; i < funcData.argInfo.size(); ++i) {
            ...
            if (not argInfo.sizeArgument) {
                ...
                std::vector<AbstractDomain::Ptr> offsets = { module_->getDomainFor(CI.getArgOperand(size), CI.getParent()) };

                std::vector<AbstractDomain::Ptr> normalizedShifts;
                for (auto&& it : offsets) {
                    normalizedShifts.emplace_back(module_->variableFactory()->af()->machineIntInterval(it));
                }
                auto bug = OutOfBoundsVisitor().visit(ptr, normalizedShifts);
                defects_[di] |= bug;
