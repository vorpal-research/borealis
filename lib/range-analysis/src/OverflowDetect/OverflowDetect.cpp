...
Constant* OverflowDetect::getSourceFile(Instruction* I){
	...
	if (SourceFiles.count(File)) {
		...
		Constant* stringConstant = llvm::ConstantDataArray::getString(*context, File, true);
...
void OverflowDetect::InsertGlobalDeclarations(){
	...
	Constant* stringConstant = llvm::ConstantDataArray::getString(*context, "Overflow occurred in %s, line %d.\n", true);
	...
	stringConstant = llvm::ConstantDataArray::getString(*context, "Truncation with data loss occurred in %s, line %d.\n", true);
