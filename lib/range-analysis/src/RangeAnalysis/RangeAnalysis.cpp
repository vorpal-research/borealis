...
void ControlDep::print(raw_ostream&) const {
}
...
void ConstraintGraph::buildValueSwitchMap(const SwitchInst *sw) {
	...
	for(auto it = sw->case_begin(); it != sw->case_end(); it++) {
		const BasicBlock *succ = it.getCaseSuccessor();
		const ConstantInt *constant = it.getCaseValue();
...
void ConstraintGraph::buildConstantVector(const SmallPtrSet<VarNode*, 32> &component, const UseMap &compusemap)
{
	...
	// Get constants inside component (TODO: may not be necessary, since components with more than 1 node never have a constant inside them)
...
bool Meet::fixed(BasicOp* op, const SmallVector<APInt, 2> *){
...
bool Meet::growth(BasicOp* op, const SmallVector<APInt, 2> *) {
	Range oldInterval = op->getSink()->getRange();
	Range newInterval = op->eval();
...
bool Meet::narrow(BasicOp* op, const SmallVector<APInt, 2> *) {
...
bool Meet::crop(BasicOp* op, const SmallVector<APInt, 2> *) {
	Range oldInterval = op->getSink()->getRange();
	Range newInterval = op->eval();
