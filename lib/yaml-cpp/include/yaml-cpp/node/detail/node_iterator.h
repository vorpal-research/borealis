...
public:
    ...
    typedef node_iterator_base<V> self;
...
public:
    ...
    template<typename W>
    node_iterator_base(const node_iterator_base<W>& rhs, typename std::enable_if<std::is_convertible<W*, V*>::value, void*>::type = nullptr) :
        m_type(rhs.m_type), m_seqIt(rhs.m_seqIt), m_mapIt(rhs.m_mapIt), m_mapEnd(rhs.m_mapEnd) {}

    node_iterator_base(const node_iterator_base&) = default;
    ...
    inline bool operator!=(const self& X) const { return ! equal(X); }
    inline bool operator==(const self& X) const { return   equal(X); }
