...
class DSCallSite {
  ...
  static void InitNH(DSNodeHandle &NH, const DSNodeHandle &Src,
                     const std::unordered_map<const DSNode*, DSNode*> &NodeMap) {
    if (DSNode *N = Src.getNode()) {
      std::unordered_map<const DSNode*, DSNode*>::const_iterator I = NodeMap.find(N);
  ...
  static void InitNH(DSNodeHandle &NH, const DSNodeHandle &Src,
                     const std::unordered_map<const DSNode*, DSNodeHandle> &NodeMap) {
    if (DSNode *N = Src.getNode()) {
      std::unordered_map<const DSNode*, DSNodeHandle>::const_iterator I = NodeMap.find(N);
      assert(I != NodeMap.end() && "Node not in mapping!");
