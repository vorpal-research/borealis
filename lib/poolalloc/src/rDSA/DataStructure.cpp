    ...
    void PopStackAndAdvance() {
      ...
      while (!Stack.empty()) {
        ...
        if (StructType *ST = dyn_cast<StructType>(SS.Ty)) {
          ...
          ArrayType *AT = cast<ArrayType>(SS.Ty);
          ++SS.Idx;
...
bool DSNode::mergeTypeInfo(Type *NewTy, unsigned Offset,
  ...
  while (ArrayType *AT = dyn_cast<ArrayType>(NewTy)) {
  ...
  if (Offset+NewTySize > Size) {
    ...
    if (Offset) {
      ...
      if (isa<StructType>(NewTy) && isa<StructType>(Ty)) {
        ...
        //and merge
        STy = StructType::get(STy->getContext(), ArrayRef<Type*>(nt));
...
///
void DSNode::markReachableNodes(std::unordered_set<const DSNode*> &ReachableNodes) const {
  if (this == 0) return;
...
void DSCallSite::markReachableNodes(std::unordered_set<const DSNode*> &Nodes) const {
...
// marked as alive...
//
static bool CanReachAliveNodes(DSNode *N, std::unordered_set<const DSNode*> &Alive,
                               std::unordered_set<const DSNode*> &Visited,
                               bool IgnoreGlobals) {
  if (N == 0) return false;
...
static bool CallSiteUsesAliveArgs(const DSCallSite &CS,
                                  std::unordered_set<const DSNode*> &Alive,
                                  std::unordered_set<const DSNode*> &Visited,
                                  bool IgnoreGlobals) {
...
void DSGraph::removeDeadNodes(unsigned Flags) {
  ...
  bool Iterate;
  std::unordered_set<const DSNode*> Visited;
  std::unordered_set<const DSCallSite*> AuxFCallsAlive;
  do {
    Visited.clear();
...
void DataStructures::print(raw_ostream&O, const Module *M) const {
...
void DataStructures::releaseMemory() {
  ...
  std::unordered_set<DSGraph*> toDelete;
  for (DSInfoTy::iterator I = DSInfo.begin(), E = DSInfo.end(); I != E; ++I) {
    I->second->getReturnNodes().clear();
    toDelete.insert(I->second);
  }
  for (std::unordered_set<DSGraph*>::iterator I = toDelete.begin(), E = toDelete.end(); I != E; ++I)
    delete *I;
