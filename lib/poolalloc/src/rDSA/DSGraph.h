...
///
class DSScalarMap {
  typedef std::unordered_map<const Value*, DSNodeHandle> ValueMapTy;
  ValueMapTy ValueMap;

  typedef std::unordered_set<const GlobalValue*> GlobalSetTy;
...
public:
  ...
  typedef std::unordered_map<const Function*, DSNodeHandle> ReturnNodesTy;
  typedef ilist<DSNode> NodeListTy;

  /// NodeMapTy - This data type is used when cloning one graph into another to
  /// keep track of the correspondence between the nodes in the old and new
  /// graphs.
  typedef std::unordered_map<const DSNode*, DSNodeHandle> NodeMapTy;

  // InvNodeMapTy - This data type is used to represent the inverse of a node
  // map.
  typedef std::unordered_multimap<DSNodeHandle, const DSNode*> InvNodeMapTy;
private:
