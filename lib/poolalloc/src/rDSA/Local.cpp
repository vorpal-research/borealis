...
namespace {
  ...
  protected:
    ...
    DSNode* createNode(Type *Ty, GlobalVariable* GV) {
      DSNode* N = new DSNode(Ty, G);
      N->addGlobal(GV);
      return N;
    }
    DSNode* createNode(Type *Ty, GlobalAlias* GA) {
      DSNode* N = new DSNode(Ty, G);
      N->addGlobal(GA);
      return N;
    }
    DSNode* createNode(Type *Ty, Function* FV) {
      DSNode* N = new DSNode(Ty, G);
      N->addFunction(FV);
      return N;
    }
...
void GraphBuilderBase::visitGetElementPtrInst(User &GEP) {
  ...
  PointerType *PTy = cast<PointerType>(GEP.getOperand(0)->getType());
...
// pointed to by NH.
void GraphBuilderGlobal::MergeConstantInitIntoNode(DSNodeHandle &NH, Type* Ty, Constant *C) {
  // Ensure a type-record exists...
  ...
  if (ConstantArray *CA = dyn_cast<ConstantArray>(C)) {
    ...
    for (unsigned i = 0, e = CS->getNumOperands(); i != e; ++i) {
      ...
      if (SL->getElementOffset(i) < SL->getSizeInBytes()) {
        ...
        outs() << "Zero size element at end of struct\n";
