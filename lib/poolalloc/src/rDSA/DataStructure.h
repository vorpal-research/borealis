...
class InstCallGraph {
  typedef std::unordered_map<const Instruction*, std::vector<const Function*> > ActualCalleesTy;
...
class DataStructures : public ModulePass {
  typedef std::unordered_map<const Function*, DSGraph*> DSInfoTy;
...
private:
  ...
  unsigned calculateGraphs(const Function *F, 
                           ...
                           unsigned &NextID,
                           std::unordered_map<const Function*, unsigned> &ValMap);
...
class TDDataStructures : public DataStructures {
  std::unordered_set<const Function*> ArgsRemainIncomplete;
...
private:
  ...
  void ComputePostOrder(const Function* F, std::unordered_set<DSGraph*> &Visited,
