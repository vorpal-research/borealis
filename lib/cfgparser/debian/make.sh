...
#
# $Id: make.sh 4249 2008-01-03 13:41:31Z michal.bukovsky $
#
# DESCRIPTION
# Packager for cfgparser library.
#
# AUTHORS
...
while [ "$#" != "0" ]; do
    case "$1" in
        --help)
            ...
            echo "    libcfgparser[-dev].control file."
            echo ""
            echo "    You can also create libcfgparser[-dev].postinst, libcfgparser[-dev].preinst,"
            echo "    libcfgparser[-dev].conffiles, libcfgparser[-dev].prerm and libcfgparser[-dev].postrm files"
...
if [ "${MODE}" = "binary" ]; then
    ...
    # bin
    mkdir -p ${DEBIAN_BASE}/usr/bin/
    cp -vd ${INSTALL_DIR}/usr/bin/cfggetvalue ${DEBIAN_BASE}/usr/bin/
   ...
   # build extra depend
    ...
    # compatibility hack for older programs which links against -lcfgparser-so
    ln -s /usr/lib/libcfgparser.so ${DEBIAN_BASE}/usr/lib/libcfgparser-so.so

    # Compose extra dependencies: we must depend on libcfgparser library with
...
if test -z "${SKIP_INSTALL}"; then
    (
        cd ..
        # install libcfgparser to the install-dir
...
# Determine library version -- we generate libcfgparser<LIBRARY_VERSION>
# and libcfgparser<LIBRARY_VERSION>-dev packages.
LIBRARY_VERSION=$(sed -n -e 's/current=\(.*\)/\1/p' \
        ${INSTALL_DIR}/usr/lib/libcfgparser.la) || exit -1
