...
template<class T, class = typename std::enable_if<!std::is_base_of<std::enable_shared_from_this<T>, T>::value>::type>
std::shared_ptr<T> copy_or_share(const T& value) {
    return std::make_shared<T>(value);
}

template<class T, class = typename std::enable_if<std::is_base_of<std::enable_shared_from_this<T>, T>::value>::type>
std::shared_ptr<T> copy_or_share(T& value) {
    return value.shared_from_this();
}

template<class T, class = typename std::enable_if<std::is_base_of<std::enable_shared_from_this<T>, T>::value>::type>
std::shared_ptr<const T> copy_or_share(const T& value) {
    return value.shared_from_this();
}
