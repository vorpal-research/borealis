...
AssemblyWriter::AssemblyWriter(formatted_raw_ostream &o, SlotTracker& Mac, TypePrinting& Tp,
                               const Module *M,
                               AssemblyAnnotationWriter *AAW)
    : Out(o), TheModule(M), Machine(Mac), TypePrinter(Tp), AnnotationWriter(AAW) {
    init();
}
...
AssemblyWriter::AssemblyWriter(formatted_raw_ostream &o, const Module *M,
                               AssemblyAnnotationWriter *AAW)
    : Out(o), TheModule(M),
      ModuleSlotTracker(new SlotTracker(M)), Machine(*ModuleSlotTracker),
      ModuleTypePrinting(new TypePrinting()), TypePrinter(*ModuleTypePrinting),
...
void printModule(Module* self, raw_ostream &ROS, AssemblyAnnotationWriter *AAW, SlotTracker& St, TypePrinting& Tp) {
    formatted_raw_ostream OS(ROS);
    AssemblyWriter W(OS, St, Tp, self, AAW);
    W.printModule(self);
}

void printNamedMDNode(NamedMDNode* self, raw_ostream &ROS, SlotTracker& St, TypePrinting& Tp) {
    formatted_raw_ostream OS(ROS);
    AssemblyWriter W(OS, St, Tp, self->getParent(), nullptr);
    W.printNamedMDNode(self);
}

void printComdat(Comdat* self, raw_ostream &ROS, SlotTracker&, TypePrinting&) {
    PrintLLVMName(ROS, self->getName(), ComdatPrefix);
    ROS << " = comdat ";

    switch (self->getSelectionKind()) {
        case Comdat::Any:
            ROS << "any";
            break;
...
void printType(Type* self, raw_ostream &OS, TypePrinting& Tp) {
    Tp.print(const_cast<Type*>(self), OS);

    // If the type is a named struct type, print the body as well.
    if (StructType *STy = dyn_cast<StructType>(const_cast<Type*>(self)))
        if (!STy->isLiteral()) {
            OS << " = type ";
...
void printValue(Value* self, raw_ostream& ROS, SlotTracker& St, TypePrinting& Tp) {
    formatted_raw_ostream OS(ROS);
    if (const Instruction *I = dyn_cast<Instruction>(self)) {
        AssemblyWriter W(OS, St, Tp, getModuleFromVal(I), nullptr);
        W.printInstruction(*I);
    } else if (const BasicBlock *BB = dyn_cast<BasicBlock>(self)) {
        AssemblyWriter W(OS, St, Tp, getModuleFromVal(BB), nullptr);
        W.printBasicBlock(BB);
    } else if (const GlobalValue *GV = dyn_cast<GlobalValue>(self)) {
        AssemblyWriter W(OS, St, Tp, GV->getParent(), nullptr);
    ...
    } else if (const MDNode *N = dyn_cast<MDNode>(self)) {
        const Function *F = N->getFunction();
        AssemblyWriter W(OS, St, Tp, F ? F->getParent() : nullptr, nullptr);
        W.printMDNodeBody(N);
    } else if (const Constant *C = dyn_cast<Constant>(self)) {
        Tp.print(C->getType(), OS);
        OS << ' ';
...
void printValueAsOperand(Value* self, raw_ostream &O, bool PrintType,
                         const Module *M, SlotTracker& St, TypePrinting& Tp) {
    // Fast path: Don't construct and populate a TypePrinting object if we
    // won't be needing any types printed.
    if (!PrintType &&
        ((!isa<Constant>(self) && !isa<MDNode>(self)) ||
         self->hasName() || isa<GlobalValue>(self))) {
        WriteAsOperandInternal(O, self, nullptr, nullptr, M);
        return;
    }

    if (PrintType) {
        Tp.print(self->getType(), O);
        O << ' ';
    }

    WriteAsOperandInternal(O, self, &Tp, &St, M);
}
